...
function buildStaleCloudKeyItems(env: NodeJS.ProcessEnv): SecretsStatusItem[] {
  return REMOVED_CLOUD_KEY_ENVS.filter((key) => env[key]?.trim()).map((key) => ({
    ...
    label: key,
    description:
      "Wird nicht mehr benutzt — UWE spricht ausschließlich den Maschinenraum-Host an. Aus der .env entfernen.",
    source: "env" as const,
    status: "set" as const,
    maskedHint: null,
    envKey: key,
...
function buildHostEnvSection(env: NodeJS.ProcessEnv): SecretsStatusSection {
  const items: SecretsStatusItem[] = [
    ...
    assessEnvSecret({
      id: "engine-service-token",
      label: "ENGINE_SERVICE_TOKEN",
      envKey: "ENGINE_SERVICE_TOKEN",
      env,
      alternateEnvKey: "ENGINE_AGENT_TOKEN",
      description: "Bearer für direkten Maschinenraum-Worker (optional).",
