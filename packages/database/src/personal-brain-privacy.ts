 ...
 * Cloud providers were removed — the Maschinenraum host is the only backend — so the
...
export const PERSONAL_BRAIN_ALLOWED_PROVIDER_MODES = ["local_engine"] as const;
...
export function assertPersonalBrainLocalOnly(provider: string): void {
  if (!isPersonalBrainContextAllowedForProvider(provider)) {
    throw new Error("Life-Brain-Kontext läuft ausschließlich über den lokalen Maschinenraum-Host.");
  }
}
