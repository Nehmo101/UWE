...
import { checkRtxHealth, type RtxHealthStatus } from "./rtxHealthcheck";

export type RtxReadinessSource = RtxHealthStatus["source"] | "connector";
...
export interface RtxReadinessOptions {
  useMock?: boolean;
...
/**
 * Unified RTX readiness — direct inference health **or** an online outbound
 * connector advertising `llm_local`. This is the single source of truth for
 * routing, admin status, AI gateway, and KI chat chips.
 */
export async function checkRtxReadiness(
  options?: RtxReadinessOptions,
): Promise<RtxReadinessStatus> {
  const direct = await checkRtxHealth({
...
export async function isRtxReadinessReady(options?: RtxReadinessOptions): Promise<boolean> {
  const status = await checkRtxReadiness(options);
  return status.ready;
}
