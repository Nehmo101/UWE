 ...
 * removed — every AI action in UWE runs on the Maschinenraum host, so there is no route
 ...
 * must get the input it needs, and the Maschinenraum host must actually be reachable.
 */
...
/**
 * The Maschinenraum host is the only backend. When it is offline there is nothing to fall
 * back to, so every request fails fast with a message that points at the cause.
 */
export function validateLocalEngineRequired(contextMode: AiContextMode, engineOnline: boolean): void {
  if (engineOnline) {
    return;
  }

  throw new AiRouterError(
    `Lokale Maschinenraum-Inference ist nicht erreichbar — „${CONTEXT_MODE_LABELS[contextMode]}" kann nicht ausgeführt werden. Bitte den Maschinenraum-Host prüfen.`,
  );
}
