 ...
 *
 * Moved out of `@uwe/atlas/rtx-asset-proposal` (the 2D Atlas package is being
 * retired). `@uwe/ai-brain` is the only consumer; behaviour and every exported
 * name are unchanged. The wire shape now lives in `./rtx-asset-schema` and is
 * re-exported here, so importers keep a single entry point.
 */

import { GOUACHE_ASSETS } from "./gouache-registry";
import type { GouacheAsset } from "./gouache-registry";
import {
