...
import { GOUACHE_ASSETS, GOUACHE_ASSET_KEYS } from "./assets";
import { BiomeKind } from "./constants";
import type { BiomeKind as BiomeKindValue } from "./constants";
import type { PlotFillAssetChoice } from "./plot-fill";
...
export interface AtlasPlotFillProposal {
  ...
  biomeKind?: BiomeKindValue;
  /** Density multiplier. UWE clamps accepted proposals before object creation. */
  density: number;
  /** Deterministic seed for fillPlotWithGouacheAssets. */
  seed: number;
...
const BIOME_VALUES = Object.values(BiomeKind) as BiomeKindValue[];
...
export function validateAtlasPlotFillProposal(
  ...
  const biomeKind =
    typeof raw.biomeKind === "string" && (BIOME_VALUES as string[]).includes(raw.biomeKind)
      ? (raw.biomeKind as BiomeKindValue)
      : undefined;
