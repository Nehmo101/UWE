...
} from "./inference/engine-worker-config";
...
export interface EngineWorkerRequestOptions {
  path: string;
  method?: string;
  body?: string;
  signal?: AbortSignal;
...
export interface EngineWorkerBoundaryConfig {
  baseUrl: string;
  token: string;
  timeoutMs: number;
  maxRetries: number;
...
export function resolveEngineBaseUrl(env: NodeJS.ProcessEnv = process.env): string | null {
  rejectClientWorkerUrl(undefined, env);
  const config = resolveEngineWorkerConfig(env);
...
export function rejectClientWorkerUrl(
  ...
  if (clientProvidedUrl?.trim()) {
    throw new EngineBoundaryError(
      "Worker-URLs dürfen nicht vom Client übergeben werden — ENGINE_BASE_URL kommt ausschließlich aus der Umgebung.",
    );
  }
...
export function resolveEngineWorkerBoundary(
  env: NodeJS.ProcessEnv = process.env,
): EngineWorkerBoundaryConfig | null {
  rejectClientWorkerUrl(undefined, env);
  ...
  if (!config.token.trim()) {
    throw new EngineBoundaryError(
      "ENGINE_SERVICE_TOKEN fehlt — Service-Token zwischen UWE und Maschinenraum-Worker ist erforderlich.",
    );
  }

  const maxRetriesRaw = env.ENGINE_MAX_RETRIES?.trim();
  ...
  return {
    ...
    maxRetries,
    hmacSecret: env.ENGINE_HMAC_SECRET?.trim() || undefined,
  };
}

export function buildEngineWorkerAuthHeaders(
  boundary: EngineWorkerBoundaryConfig,
  body?: string,
...
export function verifyEngineHmacSignature(
  body: string,
...
export async function fetchEngineWorker(
  boundary: EngineWorkerBoundaryConfig,
  options: EngineWorkerRequestOptions,
): Promise<Response> {
  ...
  for (let attempt = 0; attempt <= boundary.maxRetries; attempt += 1) {
    ...
    try {
      const response = await fetch(url, {
        method,
        headers: {
          ...buildEngineWorkerAuthHeaders(boundary, body),
  ...
  throw new EngineBoundaryError(
    `Maschinenraum-Worker unter ${boundary.baseUrl} (${kind}) nicht erreichbar nach ${boundary.maxRetries + 1} Versuchen.` +
      (lastError instanceof Error ? ` ${lastError.message}` : ""),
  );
}

export function toEngineWorkerConfig(boundary: EngineWorkerBoundaryConfig): EngineWorkerConfig {
  return {
...
/** @deprecated Use {@link toEngineWorkerConfig}. */
export const toEngineAgentConfig = toEngineWorkerConfig;
