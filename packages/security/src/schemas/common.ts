...
/** Kategorien wie im Prisma-Enum `PlayerNoteCategory` — bewusst hier gedoppelt,
 *  damit das Security-Paket nicht vom Datenbank-Paket abhaengt. */
export const playerNoteCategorySchema = z.enum([
  "session",
  "npc",
  "ort",
  "quest",
...
/** Datum des Spielabends als YYYY-MM-DD; leer erlaubt. */
const playerNoteSessionDateSchema = z
  .string()
  .trim()
  .regex(/^\d{4}-\d{2}-\d{2}$/)
  .optional()
...
export const playerNoteCreateSchema = z.object({
  ...
  category: playerNoteCategorySchema.optional(),
  sessionDate: playerNoteSessionDateSchema,
