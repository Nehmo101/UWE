...
- **Release über Release-Tags** — den Tag `uwe-vX.Y.Z` erzeugt weiterhin die Pipeline; zusätzlich baut der Workflow jetzt auch aus einem **gepushten** `uwe-v*`-Tag und prüft dabei Tag gegen `VERSION`. `pnpm release:version X.Y.Z` (`scripts/set-release-version.mjs`) setzt alle fünf Fassungsangaben inkl. `src-tauri/Cargo.toml` in einem Befehl — dasselbe Skript benutzt der Release-Build, damit die App ihre Version korrekt meldet; `pnpm release:version --check` findet Drift
