...
hardware — any PC with a GPU, Windows or Linux. It connects **outbound** to the
...
The worker was originally called **„RTX Host Connector"**. „RTX" is an NVIDIA
product line, and naming the whole engine room after one vendor's cards was
wrong twice over: UWE never required that specific hardware, and most of what
the worker does is not GPU work at all — document OCR, audio playback, Spotify
control, dictation and label printing all run on plain CPU. Any GPU that Ollama,
LM Studio or an OpenAI-compatible server can drive works, and several executors
need no GPU whatsoever.

Since 2026-08 the name is gone from the code as well, not just from the surface.
The vocabulary is now:

| Concept | Name |
|---|---|
| The worker (product name, UI) | **Maschinenraum** |
| The desktop app that configures it | **UWE Command Center** |
| Code identifiers, paths, env vars | `engine…` / `ENGINE_…` |

The machine itself is the **Maschinenraum-Host** — never „the RTX", because it
does not have to be one. The Command Center is the bridge; the Maschinenraum is
where the work happens.

### Migration from the old names

The rename is **not** backwards compatible — an existing installation needs one
pass of manual updates. See [engine-rename-migration.md](engineering/engine-rename-migration.md)
for the full checklist. In short:

| Was | Is now |
|---|---|
| `tools/uwe-rtx-connector` | `tools/uwe-engine-connector` |
| `apps/rtx-connector-client` | `apps/engine-connector-client` |
| `@uwe/rtx-connector-client` | `@uwe/engine-connector-client` |
| `RTX_BASE_URL`, `RTX_SERVICE_TOKEN`, `RTX_*` | `ENGINE_BASE_URL`, `ENGINE_SERVICE_TOKEN`, `ENGINE_*` |
| `uwe-rtx-connector.service` | `uwe-engine-connector.service` |
| `%LOCALAPPDATA%\UWE\rtx-connector-client` | `%LOCALAPPDATA%\UWE\engine-connector-client` |
| `~/.local/share/UWE/rtx-connector-client` | `~/.local/share/UWE/engine-connector-client` |

`UWE_CONNECTOR_*`, `UWE_HOST_URL` and the token prefix `uwec_…` never carried the
old brand and are unchanged. The Command Center migrates its own data directory
and cleans up the old autostart entries on first launch.
...
whose frames are NDJSON. The host never connects to the Maschinenraum machine, so **no
public port, SSH or HTTP server** is opened on the Maschinenraum side. There is no public
Maschinenraum API and no DB replication.
...
2. On the Maschinenraum machine (repo checked out, `pnpm install` done):

   ```bash
   cp tools/uwe-engine-connector/.env.example tools/uwe-engine-connector/.env
   # set UWE_HOST_URL and UWE_CONNECTOR_TOKEN
   pnpm connector:start
   ```

See `tools/uwe-engine-connector/.env.example` for all options. Transport is
selected with `UWE_CONNECTOR_TRANSPORT=queue|direct|hybrid`. On a production
Linux host, `setup-uwe-host.sh` also installs `uwe-engine-connector.service`; it is only
...
- **Hugging Face file downloads** im UWE Command Center. Downloaded files
  ...
  `HUGGINGFACE_HUB_TOKEN` from the Maschinenraum machine environment.
...
- LM Studio and llama.cpp are still discovered for operator visibility, but they
  ...
  interface/gateway, not behind the Maschinenraum connector.
...
GitHub CI has no CUPS printer or Maschinenraum connector hardware. Unit tests cover
...
Run this checklist on a homelab with UWE Host + Maschinenraum connector before relying on
physical label output:

1. **Connector env** — copy `tools/uwe-engine-connector/.env.example` to `.env` and set:
