...
| Exemptions | `/api/health`, `/api/internal`, `/api/connectors` are **always** exempt — the tunnel probes, healthcheck timer and the Maschinenraum connector are machine clients and would read a challenge page as an outage. More can be added; these cannot be removed. |
