...
Der Workflow führt `pnpm quality` (Prisma generate → lint → typecheck → test → security → secret scan → production audit → build) **vor** dem Push aus. Fehlgeschlagene Quality-Checks blockieren den PR.
