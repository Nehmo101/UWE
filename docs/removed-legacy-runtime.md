# Removed legacy runtime (Docker, Windows installer, inbound Maschinenraum-Agent)

UWE was simplified to two clear roles: an always-on **UWE Host** (Linux) and an
optional, outbound **Maschinenraum**. As part of that simplification, Docker,
the old Windows one-click host installer, and the standalone **inbound** Maschinenraum-Agent
...
- The host should run on a small always-on Linux box (an old laptop) with
  ...
  whole runtime.
- The Maschinenraum machine is an optional worker that connects **outbound** to the host.
...
| `docs/WINDOWS_INSTALLER.md`, `docs/windows-install.md`, `docs/windows-troubleshooting.md`, `docs/windows-test-checklist.md` | [docs/host-linux.md](host-linux.md), [docs/engine-connector.md](engine-connector.md). |
| `tools/uwe-engine-agent/` (package `@uwe/engine-agent`, incl. `scripts/engine-tray.ps1`) | **Removed.** The inbound Maschinenraum-Agent is no longer shipped. Use the outbound [Maschinenraum](engine-connector.md) + direct Ollama/LM Studio. |
| `packages/ai-brain/src/engine-agent-client.ts`, `packages/ai-brain/src/providers/engine-agent-provider.ts` | **Removed.** Inbound-agent LLM client/provider; ai-brain inference now uses direct Ollama/LM Studio + the connector. The Maschinenraum worker config shim stays only for the security boundary and worker/image path. |
...
- The Maschinenraum **worker** security boundary (`@uwe/security` `engine-boundary`) and the
  Maschinenraum worker/image path still resolve and LAN-validate a worker URL. Prefer
  `ENGINE_BASE_URL` / `ENGINE_SERVICE_TOKEN`; old agent-named env aliases are read only
...
- The Maschinenraum is optional; UWE is fully online without it.
...
- The standalone inbound Maschinenraum-Agent tool **and** its ai-brain LLM client/provider
  (`engine-agent-client`, `engine-agent-provider`) have been removed. LLM inference now
  uses direct Ollama/LM Studio (`AI_INFERENCE_BASE_URL`) + the outbound Maschinenraum.
  `ENGINE_BASE_URL` / `ENGINE_SERVICE_TOKEN` are the current worker URL/token
