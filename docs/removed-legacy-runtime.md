...
- The host should run on a small always-on Linux box (an old laptop) with
  `pnpm` + `systemd` (+ optional Cloudflare Tunnel). That is the whole runtime.
...
| `scripts/windows/*.ps1`, `UWE-Installieren.cmd` | — |
...
| `.github/workflows/windows-installer.yml`, the `docker-build` job in `ci.yml` | CI now runs lint, typecheck, unit/security tests, migration checks, host-setup shellcheck, connector + queue tests. |
| `scripts/windows-installer.test.ts`, Docker assertions in `scripts/selfhost.test.ts` | Tests cover the Linux host + connector path. |
