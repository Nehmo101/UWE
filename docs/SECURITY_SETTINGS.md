...
1. **Alles bleibt lokal** — es gibt keinen Cloud-Anbieter mehr (N.3). Jeder Prompt geht an den Maschinenraum-Host, also verlässt auch Welt- und Brain-Kontext den Host nicht.
...
| Secret | Feature |
|--------|---------|
| `ENGINE_SERVICE_TOKEN` | Direkter Maschinenraum-Worker / Image-Pfad |
...
|---------|-------------|----------------|----------------|
| Image Studio | Maschinenraum-Worker / Connector | DALL-E wenn `ALLOW_CLOUD` | Nur Prompt |
...
- `/admin/status` — Maschinenraum/Cloud Ampel (bestehend)
...
AI_LOCAL_ONLY=true  # ohnehin der einzige Weg — alles läuft über den Maschinenraum-Host
```
