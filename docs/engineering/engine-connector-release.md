...
1. **Versions-PR.** Ein Befehl bereitet alles vor, was maschinell bestimmbar ist:
   ...
   `apps/engine-connector-client/package.json`, `…/src-tauri/tauri.conf.json`,
...
(`.github/workflows/engine-connector-release.yml`) remains available for Linux/macOS
...
Data and uploads stay under `%LOCALAPPDATA%\UWE\engine-connector-client\host` and
...
1. Generate a Tauri updater keypair:
   ...
   ```bash
   pnpm tauri signer generate -w ~/.tauri/uwe-engine-connector.key
   ```

2. Set the **public** key in `apps/engine-connector-client/src-tauri/tauri.conf.json`
...
UWE_E2E_TAURI=1 UWE_CONNECTOR_CLIENT_URL=http://127.0.0.1:1420 \
  pnpm test:e2e e2e/engine-connector-client.spec.ts

# Label print with stub print command on the Maschinenraum host
