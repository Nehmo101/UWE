# Staging-Umgebung auf dem Maschinenraum-PC (On-Demand)
...
- **`dev`** → Staging on-demand auf dem **Maschinenraum-PC** (`test.studio` / `test.portal.uwe.example`), eigene Datenbank.

Der Prod-Host wird davon **nicht angefasst**. Staging läuft nur, wenn der Maschinenraum-PC an ist und du es manuell hochfährst.

> Warum der Maschinenraum-PC und nicht der Host: Der 4-GB-Host muss schon für **einen** Build die laufenden Apps stoppen (siehe [self-hosted-ci.md](self-hosted-ci.md) → Deploy-Runner). Ein zweites Dauer-System hätte dort keinen RAM. Der Maschinenraum-PC hat Reserven und ist ohnehin vorhanden (Ollama, Maschinenraum).

Die einsatzfertigen Vorlagen (Skripte, `.env`, Tunnel-Config) liegen unter [`deploy/engine-staging/`](../../deploy/engine-staging/); dieses Dokument erklärt Aufbau und Betrieb.

---

## Architektur-Prinzip: erreichbar bleiben, ohne inbound-Port

Der Maschinenraum-PC ist per Design **outbound-only** — „no public port, SSH or HTTP server is opened on the Maschinenraum side" ([engine-connector.md](../engine-connector.md)). Ein öffentliches `test.studio…` darf das nicht brechen.

Lösung: **`cloudflared` läuft auf dem Maschinenraum-PC selbst und wählt sich *ausgehend* zu Cloudflare ein** — genauso wie der Connector. Cloudflare routet die Test-Hostnames rückwärts durch diese ausgehende Verbindung auf `localhost:3002` / `:3003`.

```text
Browser ──▶ Cloudflare Edge ──▶ (ausgehender Tunnel) ──▶ Maschinenraum-PC localhost:3002/3003
```

**Kein einziger eingehender Port** auf dem Maschinenraum-PC. Die „outbound only"-Regel bleibt intakt — sogar strikter als beim Prod-Host, der auf `0.0.0.0` bindet.

---

## Trennung Prod ⇄ Staging

| Aspekt | Produktion (Host) | Staging (Maschinenraum-PC) |
|--------|-------------------|-------------------|
| Git-Branch | `main` | `dev` |
...
| Erreichbarkeit | Host-Tunnel → `studio`/`portal.…` | Maschinenraum-Tunnel → `test.studio`/`test.portal.…` |
...
## Einmalige Einrichtung (Maschinenraum-PC, Windows)

Voraussetzungen: Windows, Node 22, `pnpm`, Git — auf dem Maschinenraum-PC bereits vorhanden (Maschinenraum-Setup).
...
Config unter `%USERPROFILE%\.cloudflared\config.yml` — Vorlage: [`deploy/engine-staging/cloudflared-config.yml.example`](../../deploy/engine-staging/cloudflared-config.yml.example):
...
Kopiere [`deploy/engine-staging/uwe-test.env.example`](../../deploy/engine-staging/uwe-test.env.example) nach `C:\uwe-test\.env` und passe die Werte an (Secrets neu erzeugen mit `openssl rand -base64 32`). Die wichtigen Punkte:
...
Kein Windows-Service, kein Autostart — zwei Skripte unter [`deploy/engine-staging/`](../../deploy/engine-staging/). Wenn der Maschinenraum-PC aus ist, sind die Test-Hostnames einfach offline (für ein Testsystem in Ordnung).

- [`uwe-test-up.ps1`](../../deploy/engine-staging/uwe-test-up.ps1) — lädt die `.env`, pullt `dev`, migriert die Test-DB, baut prod-nah (`build:release`) und startet Studio (:3002), Portal (:3003) und den Tunnel.
- [`uwe-test-down.ps1`](../../deploy/engine-staging/uwe-test-down.ps1) — stoppt alles (per Listen-Port + `cloudflared`); `-Wipe` löscht zusätzlich die Test-DB.
...
2. Das ZIP auf den Maschinenraum-PC holen — **vom Maschinenraum aus ziehen** (Download über den Tunnel im Browser, oder `scp`/`rsync` vom Maschinenraum zum Host). Bleibt outbound-only; der Host verbindet sich nie zum Maschinenraum.
...
**Datenschutz-Hinweis:** Ein Full-Backup enthält echte Welt- **und** Life-Brain-Daten (siehe [backup-restore.md](../backup-restore.md)). In diesem Modus liegen also reale Inhalte auf dem Maschinenraum-PC. Zwei Dinge entschärfen das:
...
Default bleibt: Staging läuft auf Demo-Seed, **ohne** echte Daten auf dem Maschinenraum-PC.
...
```text
Feature-Branch
   └─▶ dev            → Maschinenraum hochziehen (uwe-test-up.ps1), inkl. Migration testen
...
- **Kein inbound-Port** auf dem Maschinenraum-PC — Tunnel und AI sind ausgehend/lokal.
...
| Datei | Zweck |
|-------|--------|
| [`deploy/engine-staging/`](../../deploy/engine-staging/) | Up/Down-Skripte, `.env`- und Tunnel-Vorlage (Windows) |
| [self-hosted-ci.md](self-hosted-ci.md) | Deploy-Runner, RAM-Grenzen des Hosts |
| [engine-connector.md](../engine-connector.md) | Outbound-Prinzip des Maschinenraum-PCs |
...
| Datum | Entscheidung |
|-------|--------------|
| 2026-07-02 | 2-Branch-Modell (`main`=Prod, `dev`=Staging). Staging **on-demand auf dem Maschinenraum-PC** statt auf dem 4-GB-Host — voll getrennt, eigene SQLite-DB, eigener outbound Cloudflare-Tunnel. Prod-Deploy-Pfad unverändert. Windows-Tooling unter `deploy/engine-staging/`. |
