 ...
 *
 *   UWE_E2E_TAURI=1 UWE_CONNECTOR_CLIENT_URL=http://127.0.0.1:1420 pnpm test:e2e e2e/engine-connector-client.spec.ts
 */
test.describe("Command Center (homelab)", () => {
  test("shell loads and reaches the Maschinenraum panel", async ({ page }) => {
    test.skip(
      ...
      "Homelab only: set UWE_E2E_TAURI=1 with connector client dev/build. See docs/engineering/engine-connector-release.md.",
    );
