# UWE Staging auf dem Maschinenraum-PC (On-Demand)

Windows-Tooling für eine **getrennte Test-Umgebung** auf dem Maschinenraum-PC neben der
Produktion auf dem UWE-Host. `dev` = Staging, `main` = Prod. Der Prod-Host wird
nicht angefasst.

Vollständiges Runbook: [`docs/engineering/staging-engine.md`](../../docs/engineering/staging-engine.md).
...
copy deploy\engine-staging\uwe-test.env.example .env   # dann Werte + Secrets anpassen
# cloudflared-config.yml.example nach %USERPROFILE%\.cloudflared\config.yml (siehe Runbook)

# Betrieb (aus deploy\engine-staging)
