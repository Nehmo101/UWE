...
Optional **outbound** worker for the machine with the hardware (Windows or Linux). It connects to
...
Maschinenraum machine, so no public port, SSH or HTTP server is opened on the Maschinenraum side.

## Quick start

1. In Studio open **System → Maschinenraum** and create a connector token.
2. On the Maschinenraum machine, with the repo checked out and `pnpm install` run:
...
See `docs/engine-connector.md` and `docs/connector-security.md` for details.
