...
describe("startOllama", () => {
  ...
  it("spawns the first existing Linux executable from PATH and fallbacks", async () => {
    ...
    const result = await startOllama({
      platform: "linux",
      env: { PATH: "/opt/tools/bin:/usr/local/bin", HOME: "/home/engine" },
      fetchImpl,
      existsImpl: (path) => path === "/home/engine/.local/bin/ollama",
      spawnImpl: (command, args) => {
        spawned.push({ command, args });
        return true;
      },
    ...
    assert.equal(result.ok, true);
    assert.equal(spawned.length, 1);
    assert.equal(spawned[0].command, "/home/engine/.local/bin/ollama");
    assert.deepEqual(spawned[0].args, ["serve"]);
