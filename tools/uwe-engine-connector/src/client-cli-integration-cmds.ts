...
export function cmdTestAudio(dataDir: string, source?: string): void {
  ...
  if (!audioCommand) {
    process.stdout.write(
      `${JSON.stringify({
        ok: false,
        message: "Kein Audio-Kommando konfiguriert (Audio-Panel im Maschinenraum-Client).",
      })}\n`,
    );
    return;
  }
...
export async function cmdTestPrint(dataDir: string, printerId?: string): Promise<void> {
  ...
  if (!trimmed) {
    process.stdout.write(
      `${JSON.stringify({
        ok: false,
        message: "Kein Drucker ausgewählt (Drucker-Panel im Maschinenraum-Client).",
      })}\n`,
    );
    return;
  }
...
export async function cmdTestImage(dataDir: string, prompt?: string): Promise<void> {
  ...
  if (!imageCommand) {
    process.stdout.write(
      `${JSON.stringify({
        ok: false,
        message: "Kein Image-Kommando konfiguriert (Bild-Panel im Maschinenraum-Client).",
      })}\n`,
    );
    return;
  }
