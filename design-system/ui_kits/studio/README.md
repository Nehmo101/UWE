...
- **Heute** — the Daily Admin cockpit: stat tiles (Capture inbox, projects, next session), an "Braucht Aufmerksamkeit" queue (World Inspector findings, backup age, broken wikilinks), quick-access chips, and a KI/Maschinenraum status panel.
...
`PageTypeBadge`, `EngineStatusBadge`, `SecretReveal`, `SidebarNav`, `Brand`) from
