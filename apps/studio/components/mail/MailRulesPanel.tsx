...
export function MailRulesPanel() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(studioApiUrl("/api/admin/mail/rules"), { headers: { Accept: "application/json" } });
    ...
    const actions =
      action === "move:archive"
        ...
        : action === "move:trash"
          ...
          : [{ type: action }];
    const response = await fetch(studioApiUrl("/api/admin/mail/rules"), {
      method: "POST",
  ...
  async function toggleRule(rule: RuleVM) {
    await fetch(studioApiUrl(`/api/admin/mail/rules/${rule.id}`), {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteRule(id: string) {
    await fetch(studioApiUrl(`/api/admin/mail/rules/${id}`), { method: "DELETE" });
    await load();
  }

  async function addVip() {
    if (!vipAddress.trim()) return;
    await fetch(studioApiUrl("/api/admin/mail/vips"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function removeVip(id: string) {
    await fetch(studioApiUrl(`/api/admin/mail/vips?id=${encodeURIComponent(id)}`), { method: "DELETE" });
    await load();
  }
