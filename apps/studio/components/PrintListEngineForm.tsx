...
export function PrintListEngineForm({ worldSlug, printListId, returnTo, printers, defaultPrinterKey, totalCopies, labelCount }: Props) {
  ...
  return (
    <form action={enqueueLabelPrintAction} className="flex flex-col gap-4">
      ...
      <p className="text-sm text-muted-foreground">
        Sendet alle {labelCount} Label{labelCount === 1 ? "" : "s"} mit insgesamt {totalCopies} Kopie{totalCopies === 1 ? "" : "n"} als einen Maschinenraum-Batch-Job.
      </p>
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="print-list-engine-printer">Drucker</Label>
        <select
          id="print-list-engine-printer"
          name="printerKey"
          required
      ...
      <div className="flex flex-col gap-1.5">
        <Label htmlFor="print-list-engine-format">Format</Label>
        <select id="print-list-engine-format" name="format" defaultValue="pdf" className={NATIVE_SELECT_CLASS}>
          <option value="pdf">PDF</option>
          <option value="html">HTML</option>
        </select>
      </div>
      <Button type="submit" disabled={!canSubmit} className="self-start">
        Gesamte Liste an den Maschinenraum senden
      </Button>
    </form>
  );
}
