...
export type SoundboardEngineAction = "play" | "stop" | "stop_all" | "volume";

const ACTION_JOB_TYPES: Record<SoundboardEngineAction, ConnectorJobType> = {
  play: "sound_play",
  stop: "sound_stop",
...
export interface SoundboardEngineInput {
  action: SoundboardEngineAction;
  buttonId?: string;
  sourceUrl?: string;
  title?: string;
  volume?: number;
...
export interface SoundboardEngineDeps {
  service: {
...
function defaultDeps(): SoundboardEngineDeps {
  return {
...
export async function dispatchSoundboardEngine(
  worldSlug: string,
  input: SoundboardEngineInput,
