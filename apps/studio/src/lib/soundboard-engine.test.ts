...
import { dispatchSoundboardEngine, type SoundboardEngineDeps } from "./soundboard-engine";

function makeDeps(options: {
  direct?: boolean;
  queueEnabled?: boolean;
  dispatch?: SoundboardEngineDeps["registry"]["dispatch"];
} = {}): { deps: SoundboardEngineDeps; enqueued: EnqueueConnectorJobInput[] } {
...
describe("dispatchSoundboardEngine", () => {
  it("delivers directly without enqueueing", async () => {
    const { deps, enqueued } = makeDeps();
    const response = await dispatchSoundboardEngine(
      "terra",
  ...
  it("falls back to the queue before Direct acceptance", async () => {
    ...
    const response = await dispatchSoundboardEngine("terra", { action: "stop_all" }, deps);
  ...
  it("never enqueues after Direct acceptance", async () => {
    ...
    const response = await dispatchSoundboardEngine("terra", { action: "volume", volume: 40 }, deps);
  ...
  it("does not queue through a Direct-only connector", async () => {
    ...
    const response = await dispatchSoundboardEngine("terra", { action: "stop" }, deps);
