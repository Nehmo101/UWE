...
/**
 * Kampagnen-Verwaltung einer Welt.
 *
 * Die Kampagne war überall im Cockpit ein Filter — in der Kontextspalte, in den
 * Import-Zielen, im Backup-Formular — aber es gab keine Stelle, an der eine
 * entsteht: angelegt wurden sie nur vom Seed. Das ist diese Seite.
 */
...
export default async function WorldCampaignsPage({ params, searchParams }: Props) {
  ...
  const campaigns = await createCampaignService(prisma).listOverview(worldSlug);
  const campaignsPath = `/worlds/${worldSlug}/campaigns`;

  return (
    <>
      <ShellBreadcrumb
        items={worldSectionBreadcrumb(world.name, worldSlug, "Kampagnen", campaignsPath)}
      />
      <ShellContextPanel>
        {/*
      ...
      <PageHeader
        title="Kampagnen"
        summary={`Kampagnen von „${world.name}" anlegen, umbenennen und löschen. Die Kampagne bündelt Seiten, Sessions, Medien und Notizen — überall dort ist sie der Filter „Kampagne".`}
      />
      ...
      {campaigns.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {campaigns.map((campaign) => {
            const totalContent =
              campaign.counts.pages +
              campaign.counts.gameSessions +
              campaign.counts.assets +
              campaign.counts.characters;
            ...
            return (
              <Card key={campaign.id}>
                ...
                <CardContent className="flex flex-col gap-4">
                  <form action={renameCampaignAction} className="flex flex-col gap-3">
                    <input type="hidden" name="worldSlug" value={worldSlug} />
                  ...
                  <details className="rounded-md border border-border p-3">
                    ...
                    <div className="mt-3 flex flex-col gap-3">
                      <p className="text-sm text-muted-foreground">
                        {totalContent > 0
                          ? `Die ${totalContent} zugeordneten Inhalte (Seiten, Sessions, Medien, Charaktere) bleiben erhalten und gehören danach direkt zur Welt.`
                          : "Dieser Kampagne ist noch nichts zugeordnet."}
                      </p>
