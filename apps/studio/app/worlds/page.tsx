...
export default async function WorldsPage() {
  const [worlds, user] = await Promise.all([getAppRepository().listWorlds(), getCurrentAuthUser()]);
  ...
  return (
    <StudioShell breadcrumb={<BreadcrumbTrail items={[{ label: "Heute", href: "/today" }, { label: "Welten" }]} />}>
      ...
      {canCreateWorld ? <section className="mb-6 rounded-lg border border-border p-4"><CreateWorldForm /></section> : null}
      {worlds.length === 0 ? <EmptyState title="Noch keine Welten" description="Erstelle oben deine erste Welt." /> : (
        <section><h2 className="mb-4 text-lg font-semibold">Deine Welten</h2>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{worlds.map((world) => (
