...
const CONNECTOR_LAUNCHER_REL: &str = "tools/uwe-engine-connector/src/desktop-launcher.ts";
const CONNECTOR_CLIENT_CLI_REL: &str = "tools/uwe-engine-connector/src/client-cli.ts";
const HUGGINGFACE_CLI_REL: &str = "tools/uwe-engine-connector/src/huggingface-cli.ts";
...
const APP_NAME_DIR: &str = "engine-connector-client";
/// Datenverzeichnis vor dem Maschinenraum-Rebranding. Bleibt als Literal stehen,
/// damit `migrate_legacy_app_data_dir` Bestandsinstallationen findet.
const LEGACY_APP_NAME_DIR: &str = "rtx-connector-client";
...
struct ConnectorClientConfig {
    ...
    /// lives only on the Maschinenraum client. All defaulted for forward-compatibility
...
fn connector_app_data_dir() -> Result<PathBuf, String> {
    ...
    let vendor_dir = base_dir.join(APP_VENDOR_DIR);
    let app_dir = vendor_dir.join(APP_NAME_DIR);
    migrate_legacy_app_data_dir(&vendor_dir, &app_dir);
    Ok(app_dir)
}

/// Zieht die Daten aus dem Verzeichnis der RTX-Namensgebung an den neuen Ort.
///
/// Vor dem Rebranding lagen Konfiguration, Tokens und Modellprofile unter
/// `…/UWE/rtx-connector-client`. Ohne diesen Umzug startet eine bestehende
/// Installation nach dem Update in einem leeren Verzeichnis und verlangt die
/// komplette Ersteinrichtung noch einmal.
///
/// Der Umzug laeuft genau einmal: nur wenn das neue Verzeichnis noch fehlt und
/// das alte existiert. Schlaegt das Umbenennen fehl — etwa weil beide auf
/// verschiedenen Laufwerken liegen oder eine Datei offen ist — bleibt das alte
/// Verzeichnis unangetastet liegen und die App faellt auf die Ersteinrichtung
/// zurueck. Das ist unschoen, aber nie datenverlustig, deshalb kein harter
/// Fehler.
fn migrate_legacy_app_data_dir(vendor_dir: &Path, app_dir: &Path) {
    if app_dir.exists() {
        return;
    }
    let legacy_dir = vendor_dir.join(LEGACY_APP_NAME_DIR);
    ...
    match fs::rename(&legacy_dir, app_dir) {
        Ok(()) => {
            eprintln!(
                "[uwe] Datenverzeichnis von '{}' nach '{}' uebernommen.",
                legacy_dir.display(),
                app_dir.display()
            );
        }
        Err(error) => {
            eprintln!(
                "[uwe] Datenverzeichnis '{}' konnte nicht uebernommen werden: {error}",
                legacy_dir.display()
            );
        }
    }
...
#[cfg(target_os = "linux")]
const AUTOSTART_DESKTOP_FILE: &str = "uwe-engine-connector-client.desktop";

/// Autostart-Datei vor dem Maschinenraum-Rebranding. Wird bei jedem Sync
/// entfernt, sonst startet eine Bestandsinstallation die App nach dem Update
/// zweimal — einmal ueber den alten und einmal ueber den neuen Eintrag.
#[cfg(target_os = "linux")]
const LEGACY_AUTOSTART_DESKTOP_FILE: &str = "uwe-rtx-connector-client.desktop";
...
#[cfg(target_os = "linux")]
fn remove_legacy_linux_autostart() {
    let Some(base) = dirs::config_dir() else {
        return;
    };
    let legacy = base.join("autostart").join(LEGACY_AUTOSTART_DESKTOP_FILE);
