...
function summarize(user: CommandCenterUser): string {
  ...
  if (user.aiAccess) granted.push("Maschinenraum-KI");
...
export function UsersPanel() {
  ...
  // Die Maschinenraum-KI ist beim Anlegen aus, wie jedes andere Recht auch. Wer sie
  ...
  async function toggleAi(user: CommandCenterUser, enabled: boolean) {
    ...
    try {
      ...
      if (!result.ok) throw new Error(result.message ?? "Maschinenraum-KI konnte nicht geändert werden.");
      setMessage(
        `„${user.displayName}" ${enabled ? "darf jetzt" : "darf nicht mehr"} die Maschinenraum-KI benutzen.`,
      );
      await refresh();
    } catch (nextError) {
      setError(toMessage(nextError));
                  ...
                  <div className="connector-checkbox-row">
                    ...
                    <label className="connector-checkbox">
                      <input
                        ...
                        aria-label={`Maschinenraum-KI für ${user.displayName}`}
                      />
                      Maschinenraum-KI
                    </label>
                    <label className="connector-checkbox">
