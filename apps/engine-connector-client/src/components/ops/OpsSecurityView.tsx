...
export function OpsSecurityView({ data }: { data: unknown }) {
  ...
  const engineToken = record(env.engineServiceToken);
        ...
        <CardContent>
          <Fields
            items={[
              ...
              {
                label: text(engineToken.envKey, "ENGINE_SERVICE_TOKEN"),
                value: text(engineToken.message),
                hint: `erforderlich: ${flag(engineToken.required)} · gesetzt: ${flag(engineToken.configured)}`,
              },
            ]}
          />
        </CardContent>
