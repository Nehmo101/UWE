...
- bestehende Maschinenraum-, Modell-, Download-, Drucker- und Runner-Funktionen
...
- Tauri 2 shell with local JSON config in `%LOCALAPPDATA%/UWE/engine-connector-client/`
  (Windows) bzw. `~/.local/share/UWE/engine-connector-client/` (Linux)
...
- **Desktop-Shell**: Tray-Icon mit Öffnen/Beenden, Start im Tray/minimierter Start
  ...
  XDG-Autostart-Eintrag (`~/.config/autostart/uwe-engine-connector-client.desktop`).
...
- **Hugging Face Downloads**: Der Download-Tab kann einzelne Modell-Dateien aus
  ...
  `%LOCALAPPDATA%/UWE/engine-connector-client/models/huggingface/` und werden danach
...
- **Studio-Workflow-Standards**: Studio bietet auf `/system/engine-connector` einen
...
- **Autostart**: XDG-Autostart-Eintrag in
  `~/.config/autostart/uwe-engine-connector-client.desktop` (Toggle in den
...
- **Config/Daten**: `~/.local/share/UWE/engine-connector-client/` (config.json,
...
Tauri UI  →  start/stop  →  node --import tsx tools/uwe-engine-connector/src/desktop-launcher.ts
                                      ↓
                              ConnectorRunner (outbound heartbeat + queue/direct/hybrid transport)

Tauri UI  →  downloads   →  node --import tsx tools/uwe-engine-connector/src/huggingface-cli.ts
                                      ↓
...
and opens no listening port on the Maschinenraum machine.
...
| P6 | Legacy Maschinenraum-Agent cleanup, connector queue provider |
